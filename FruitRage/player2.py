...
class Fruit_Rage:
    ...
    def valid_move(self , visited,board, start_row):
    ...
    def compute_neighbours(self,x, y, visited,board):
        ...
        return (count * count, x, y,covered)
    ...
    def generate_moves(self,board):
        visited = np.zeros((self.N+2,self.N+2),dtype=bool)
        next_move =self.valid_move(visited, board,1)
        moves =[]
        while next_move[0]:
            moves.append(self.compute_neighbours(next_move[1][0],next_move[1][1], visited,board))
            next_move = self.valid_move(visited, board,next_move[1][0])
        moves.sort(key=itemgetter(0), reverse=True)
        if len(moves) >30:
            n_trunc =len(moves) -30
            del moves[-n_trunc:]
        return moves
